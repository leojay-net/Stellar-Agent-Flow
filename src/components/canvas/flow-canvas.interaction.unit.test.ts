...
describe("flow canvas interactions", () => {
    ...
    it("adds dropped agents to canvas", () => {
        ...
        expect(flowState.addAgentToCanvas).toHaveBeenCalledTimes(1);
        const [agentArg, positionArg] = flowState.addAgentToCanvas.mock.calls[0] as [
            { id?: string },
            { x?: number; y?: number }
        ];
        expect(agentArg.id).toBe("stellar-asset-pricer");
        expect(positionArg).toHaveProperty("x");
        expect(positionArg).toHaveProperty("y");
    });
